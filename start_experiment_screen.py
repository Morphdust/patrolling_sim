...
def loadInitPoses():
    try:
        ...
        ConfigIP.read(f"{dirname}/params/initial_poses.txt")
...
def save_exp_params_file(params_dict, anomaly_loc):
    ...
    hostname = os.uname()[1]
    filename = f"{time_string}_launch_params.txt"
...
def gen_param_dict(MAP, NROBOTS, INITPOS, ALG_SHORT, LOC_MODE, NAV_MODULE, GWAIT, COMMDELAY, TERM, TIMEOUT, CUSTOM_STAGE, SPEEDUP):
    return {
        "MAP": MAP,
        "NROBOTS": NROBOTS,
        "INITPOS": INITPOS,
...
def find_anomaly_location():
    anomaly_file_dir = "params/anomaly/anomaly_params.txt"

    with open(anomaly_file_dir, 'r') as file:
        lines = file.readlines()
    anomaly_loc = 'ERROR'
    if len(lines) >= 15:
        line_15 = lines[14]  # Indexing starts from 0, so line 15 is at index 14
        anomaly_loc = line_15
    else:
        print("The anomaly file doesn't have 15 lines.")
...
#    os.system("rosparam set /lost_message_rate "+LOSTMSGRATE)
    ...
    for i in range(0,int(NROBOTS)):
        ...
        cmd = 'roslaunch patrolling_sim '+robot_launch+' robotname:=robot_'+str(i)+' mapname:='+MAP+' '
        ...
        os.system('screen -S robot_'+str(i)+' -d -m '+cmd+'')
    ...
    for i in range(0,int(NROBOTS)):
        ...
        if (ALG_SHORT=='MSP'):
            ...
            cmd = 'rosrun patrolling_sim '+ALG+' __name:=patrol_robot'+str(i)+' '+MAP+' '+str(i)+ ' '

        print(cmd)

        os.system('screen -S patrol_'+str(i)+' -d -m '+cmd+'')
    ...
    print(f"Experiment started at {strinittime}")
